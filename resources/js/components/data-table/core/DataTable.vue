...
const tableColumns = computed<ColumnDef<TData, any>[]>(() => {
    if (internalDeletedMode.value) {
        ...
        if (actionsColumnIndex > -1) {
            ...
            return [...allColumns, deletedAtColumn];
        }
    }
...
<template>
    <div :class="containerClasses">
        <DataTableToolbar
            ...
            :enableDeletedModeToggle="props.enableDeletedModeToggle"
            :deletedMode="internalDeletedMode"
        ...
        <div
            v-if="props.showBulkActions && props.enableRowSelection && table.getSelectedRowModel().rows.length > 0 && props.bulkActions && props.bulkActions.length > 0"
            class="bg-muted flex items-center justify-between rounded-md p-3 shadow"
        >
            <span class="text-muted-foreground text-sm">{{ table.getSelectedRowModel().rows.length }} fila(s) seleccionada(s)</span>
            <div class="space-x-2">
                <button
        ...
        <div v-if="isError && queryError" class="rounded-md border border-red-600 bg-red-50 p-4 text-red-600 dark:bg-red-900/30 dark:text-red-300">
        ...
        <ExportDialog
            ...
            @update:isOpen="isExportDialogOpen = $event"
            :formats="props.exportConfig.formats"
